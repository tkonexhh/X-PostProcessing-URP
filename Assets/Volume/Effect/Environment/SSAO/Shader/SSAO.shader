...
Shader "Hidden/PostProcessing/Environment/SSAO"
{
    Properties
    {
    ...
    #define SAMPLE_RADIUS  _Params.x
    #define SAMPLE_COUNT   _Params.y
    #define INTENSITY      _Params.z
    #define BLUR_RADIUS    _Params.w
    ...
    float3 ReconstructNormal(float2 uv, float depth, float3 vpos, float2 p11_22, float2 p13_31)
    {
        ...
        #else
            float2 delta = _MainTex_TexelSize.xy * 2.0;
    ...
    {
        return uv * SCREEN_PARAMS.xy * 1;// DOWNSAMPLE;
    ...
    half4 Frag(VaryingsDefault input): SV_Target
    {
        ...
        for (int s = 0; s < int(SAMPLE_COUNT); s++)
        {
            ...
            v_s1 *= sqrt((s + 1.0) * rcpSampleCount) * SAMPLE_RADIUS;
    ...
    {
        float4 p0 = GetScreenColor(uv);
        float4 p1a = GetScreenColor(uv - delta * 1.3846153846);
        float4 p1b = GetScreenColor(uv + delta * 1.3846153846);
        float4 p2a = GetScreenColor(uv - delta * 3.2307692308);
        float4 p2b = GetScreenColor(uv + delta * 3.2307692308);
    ...
    {
        float4 p0 = GetScreenColor(uv);
        float4 p1 = GetScreenColor(uv + float2(-delta.x, -delta.y));
        float4 p2 = GetScreenColor(uv + float2(delta.x, -delta.y));
        float4 p3 = GetScreenColor(uv + float2(-delta.x, delta.y));
        float4 p4 = GetScreenColor(uv + float2(delta.x, delta.y));
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 uv = input.uv;
                float2 delta = float2(_MainTex_TexelSize.x * rcp(BLUR_RADIUS) * 2.0, 0.0);
        ...
        Pass
        {
            ...
            {
                float2 uv = input.uv;
                float2 delta = float2(0.0, _MainTex_TexelSize.y * rcp(BLUR_RADIUS) * 2.0);
                return Blur(uv, delta);
            }
        ...
        Pass
        {
            ...
            float4 FragCombine(VaryingsDefault input): SV_Target
            {
                float2 uv = input.uv;
                float2 delta = _MainTex_TexelSize.xy * rcp(1);
                half ao = 1.0 - min(0.8, BlurSmall(uv, delta));
                // return ao;
                half3 SceneColor = SAMPLE_TEXTURE2D(_ScreenTex, sampler_ScreenTex, input.uv) ;
                // ao = pow(ao, INTENSITY);
                // half3 SceneColor = GetScreenColor(input.uv);
                return half4(SceneColor * ao, 1);
            }
        ...
        Pass
        {
            ...
            float4 FragDebug(VaryingsDefault input): SV_Target
            {
                float2 uv = input.uv;
                float2 delta = _MainTex_TexelSize.xy * rcp(1);
                half ao = 1.0 - min(0.8, BlurSmall(uv, delta));
