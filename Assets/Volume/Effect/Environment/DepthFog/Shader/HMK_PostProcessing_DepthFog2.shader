Shader "Hidden/PostProcessing/DepthFog2"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(postFrag i): SV_Target
            {
                ...
                float dis = length(CtoV);
                float ConcentrationByDistance = saturate((dis - fogEnableDistance) / fogEnableDistanceArea);
                ...
                //fogPower += dis * _DisFogStrength * 0.01f;
                float3 lightDir = GetMainLight().direction;
                float3 eyeDir = -normalize(CtoV);
                float scattingValue = saturate(dot(lightDir,eyeDir)); 
                //float scattingValue = ((dot(lightDir,eyeDir) * _SunScatting) + 1) * 0.5f;
                float3 fogColor = lerp(_ColdColor,_WarmColor,scattingValue);
                
                
                //col.rgb += fogPower * _DepthFogColor.rgb;
                col.rgb = lerp(col.rgb, fogColor, saturate(fogPower));
                //return 1;
                return col;
            }
            ENDHLSL
